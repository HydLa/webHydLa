...
$(document).ready(function() {
	...
	plot_settings = browser_storage.getItem('plot_settings'); // #HOR: この 'plot_settings' とは何か
	if (plot_settings == null) {
		plot_settings = {};
	} else {
		plot_settings = JSON.parse(plot_settings); // #HOR: over-writing of variable
	}

	// if (plot_settings.plotInterval == undefined) // #HOR: excluding this would ignore browser memory
		plot_settings.plotInterval = 0.1;
	// if (plot_settings.backgroundColor == undefined) // #HOR: "load: localStorage" get data from browser memory
		plot_settings.backgroundColor = "#1D3156"; // #HOR: change; was originally set to black
	if (plot_settings.lineWidth == undefined)
		plot_settings.lineWidth = 1;
	if (plot_settings.scaleLabelVisible == undefined)
		plot_settings.scaleLabelVisible = true;
	// if (plot_settings.twoDimensional == undefined)
		// plot_settings.twoDimensional = false;
		plot_settings.twoDimensional = true; // #HOR: start from 2D mode
	if (plot_settings.autoRotate == undefined)
		plot_settings.autoRotate = false;
	if (plot_settings.animate == undefined)
		plot_settings.animate = false;
	if (plot_settings.seek == undefined)
		plot_settings.seek = 0;
...
function initVariableSelector(hydat) {
	...
	// HOR: made commented out code visible
	// var guard_list ={x:["x", "xSWON"]};
	// guard_list = hydat.guards;
	// HOR: ここでguard_listを作成せずとも、browser_storage.getItem(hydat.guards);でアクセスできる？
	...
	if (settingsForCurrentHydat != null)
	{
		...
		// console.log("settingsForCurrentHydat.plot_line_settings[0].x " + settingsForCurrentHydat.plot_line_settings[0].x);
		// HOR: settingsForCurrentHydat.plot_line_settings[0].x　で、x軸に指定されている変数を取得できる。
		// つまり、これはhydatのデータではなく、ブラウザ上で指定された軸の変数を取得する
		...
		for (var i in line_settings)
		{
			...
			for (guard in guard_list) {
				var guard_tokens = guard.split("=");
				console.log(guard_tokens);
				var guard_lhs = sample_token[0].replace('-', '');
				var guard_rhs = sample_token[1];
				if (line_settings[i].y == guard_lhs) {
					addNewLineWithIndexGuard("t", "y", "0", guard_rhs);
				}
			}
			*/
