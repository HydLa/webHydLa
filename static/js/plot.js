...
function translate_phase(phase) {
	if (phase.type=="PP") {
		phasse.time.time_point = parseValue(phase.time.time_point);
	} else {
	...
	try {
		while(true) {
			...
			if (phase.children.length == 0) {
				...
				var material;
				if(!line.settings.dashed)
					material = new THREE.LineBasicMaterial({linewidth: width, color:color[current_param_idx]});
				else {
		...
		showToast("Plot failed: " + ex.name + "(" + ex.message + ")", 5000, "red darken-4");
		line.plotting = false;
		checkAndStopPreloader();
	}
}
	...
	if(time_prev != time) {
		...
		for (i=0; i<graph_scene.children.length-1; i++) {
			if (graph_scene.children[i].type == "Line") {
				...
				graph_scene.children[i+1].position.set(animation_line[arr][time].x,
...
function guard() {
	...
	g_material = new THREE.MeshBasicMaterial({color: 0x0000ff, wireframe: true});
