...
function loadHydat(hydat)
{
	try
	{
		...
		current_hydat = hydat;
		// hydatの中身を点検するだけなのか、書き換えるのか
		translate(hydat);
		// 
		parameter_setting(current_hydat.parameters);
		// シミュレーション画面の左下にファイル名を表示するための関数
	...
	// HOR: おそらく、描画をやり直す前にクリアする
	clearPlot();
	// 
	initVariableSelector(hydat);
	update_axes(true);
	guard_list = current_hydat.guards; // HOR: added this line to make guards accesible
}
...
function modifyNameLabel(name)
{
	...
	var text2 = "file name: " + text; // HOR: changed to check functionality
	ctx.fillText(text2, 0, canvas.height - 50);
}
...
function replot_all()
{
	...
	for(var i in plot_lines)
	{
		...
		replot(plot_lines[i]); // #HOR: plot_lines[i] がおそらく軌跡を表している
	}
	time = 0;
}
...
// #HOR: its probably this function thats doing the drawing
	...
	try
	{
		while (true)
		{
			...
			if(phase.children.length == 0)
			{
				...
				s_geometry = new THREE.SphereGeometry(0.3);
				s_material = new THREE.MeshBasicMaterial( { color:0xf3b927, wireframe:true } );
...
var xAxisColorBase = {r:1.0, g:1.0, b: 1.0}; // #HOR: modifying this would change the color of the axis
var yAxisColorBase = {r:1.0, g:1.0, b: 1.0};
var zAxisColorBase = {r:1.0, g:1.0, b: 1.0};
